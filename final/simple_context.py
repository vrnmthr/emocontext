...
class SimpleContext(nn.Module):
    ...
    def forward(self, batch, seq_lens):
        """
        :param batch: python list of tokenized sentences with (batch_size, 3, seq_len)
        :param seq_lens: tensor of sequence lengths (batch_size, 3)
        :returns: (batch_size x 2) logits for each class
        """
        # have to manually chunk the sentences since it is not a torch tensor
        chunked = [[convo[i] for convo in batch] for i in range(3)]
        lchunked = torch.chunk(seq_lens, 3, 1)

        # embeddings[i] stores (batch_size, 1024) embeddings for the ith sentence across the batch
        embeddings = ()
        for i in range(3):
            # (batch_size, seq_len) batched sentences
            lens = torch.transpose(lchunked[i], 0, 1)[0]
            embed = self.embed_single(chunked[i], lens)
            embeddings = embeddings + (embed,)

        # concatenate across the first dimension to get (batch_size, 1024*3) for classification
...
def train():
    ...
    model = SimpleContext(device=DEV)
    print(model)
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        for mode in modes:
            ...
            for batch in tqdm(dataloaders[mode], desc='{}:{}/{}'.format(mode, epoch, NUM_EPOCHS)):
                # use all sentences!!
                sentences = batch['item']
                # print(sentences)
                seq_lens = torch.Tensor([[len(s) for s in convo] for convo in sentences]).to(DEV)
                # print(seq_lens)
...
def main():
    ...
    if args.restore is not None:
        ...
        print("loading model...")
        folder = "saved_runs/simple_context/run{}".format(args.restore)
        ...
        run_id = 0
        while os.path.isdir("saved_runs/simple_context/run{}".format(run_id)):
            run_id += 1
        folder = "saved_runs/simple_context/run{}".format(run_id)
